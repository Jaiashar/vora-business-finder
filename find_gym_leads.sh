...
# Vora Gym & Fitness Finder
# Scrapes gyms, bootcamps, personal trainers, stretch studios, coaches, etc.
# from Google Maps for B2B outreach
...
COMPLETED_FILE="$SCRIPT_DIR/completed_gym_cities.txt"
TEMP_QUERIES="$SCRIPT_DIR/.temp_gym_queries_$$.txt"

# Gym & Fitness business types to search for
BUSINESS_TYPES=(
    "gym"
    "fitness center"
    "CrossFit gym"
...
run_scraper() {
    ...
    print_msg "$GREEN" "Starting gym/fitness scraper for: ${city}"
...
main() {
    ...
    echo ""
    print_msg "$GREEN" "╔══════════════════════════════════════════╗"
    print_msg "$GREEN" "║    Vora Gym & Fitness Finder             ║"
    print_msg "$GREEN" "╚══════════════════════════════════════════╝"
    echo ""
    ...
    if check_completed "$city"; then
        ...
        if [[ "$force_mode" == "true" ]]; then
            ...
            echo ""
            if [[ ! $REPLY =~ ^[Yy]$ ]]; then
                print_msg "$BLUE" "Skipping. Use a different city or delete the entry from completed_gym_cities.txt"
                exit 0
            fi
        fi
        echo ""
    ...
    if run_scraper "$city" "$output_file" "$test_mode"; then
        ...
        if [[ -f "$output_file" ]]; then
            ...
            if [[ $count -gt 0 ]]; then
                ...
                print_msg "$GREEN" "  Gym/fitness leads with valid emails: ${count}"
            else
